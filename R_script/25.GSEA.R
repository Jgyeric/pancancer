...
setwd("D:\\Rdata\\panCancer\\25.GSEA")      #设置工作目录
gene="CENPI"                               #基因名称
gmtFile="c5.all.v7.4.symbols.gmt"          #基因集文件
...
for(i in files){
  ...
  write.table(kkTab,file=paste0("GO.",CancerType,".txt"),sep="\t",quote=F,row.names = F)
  
  #输出富集的图形
  termNum=5
  if(nrow(kkTab)>=termNum){
    gseaplot=gseaplot2(kk, row.names(kkTab)[1:termNum],base_size =8)
    pdf(file=paste0("GO.",CancerType,".pdf"),width=8,height=6)
    print(gseaplot)
    dev.off()
  }
}